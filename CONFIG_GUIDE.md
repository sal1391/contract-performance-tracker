# the legacy app Configuration Guide

## The Two Switches

the legacy app has two main switches at the top of `config.py` that control how the app behaves:
...
- **`True`** — A login screen appears. Users must sign in with corporate credentials
  and have the right role (`App:Sales`) to get access. This is the locked front
  door with a keycard.
